...
Applies the `compute_sky_visibility()` function to the DSM to compute sky visibility above two elevation angles (15° and 30°). It also computes a variant of the 30° visibility where a high tower has been removed to simulate the landscape without obstructions. Make sure to set the correct `zscale`, the ratio between the x and y spacing (which should be equal) and the z-spacing, zee [`rayshader` documentation](https://www.rayshader.com/reference/ray_shade.html). In this case `zscale = 0.5/1 = 0.5`, because the spacing between grid cells is 0.5m and the elevation is measured in meters.
